import { Box, Input, InputGroup, InputLeftAddon, Spinner, useColorMode } from "@chakra-ui/core";
...
import { Field, Form, FormRenderProps, useField, useFormState } from "react-final-form";
...
async function validate(data: Partial<MainFormData>): Promise<Partial<MainFormData>> {
...
function useInitialValues(defaultValue: MainFormData): [MainFormData, Function] {
        ...
        return (
          <PageContainer spacing="0px">
            <Stack
              ...
              {...rest}
            >
              <Stack onSubmit={formProps.handleSubmit} align="center" w="100%" as="form">
                <AuthProvider
                  onError={console.error}
                  onLogin={async (user) => {
                    setShouldLogin(false);
                    console.log("called on login, redirecting");
  ...
  if (submitting || loading) {
    return (
      <Stack
        ...
        {...rest}
      >
        <Box>{loading ? "logging in" : "Generating the sdk package"}</Box>
        <Spinner />
      </Stack>
    );
  }
  ...
  if (!submitting) {
    return (
      ...
      >
        ...
        <Stack w={["100%", null, null, "auto"]} position="relative" spacing="10px">
          <Label>
...
const Label = (props) => {
  ...
  return <Box color={{ light: "gray.500", dark: "gray.200" }[colorMode]} {...props} />;
};
...
export const ValidationError = ({ name, ...rest }) => {
  ...
  if (touched && error) {
    return (
      <Box bottom="-50px" position="absolute" minH="40px" color="red.500" {...rest}>
        {error}
      </Box>
    );
  }
