...
After generating the client using the cli or the form at [genqlx.now.sh](https://genqlx.vercel.app) you can initialize it passing your options
...
await client.query({
    repository: [{ name: 'genqlx', owner: 'remorses' }, { ...everything }],
})
```
