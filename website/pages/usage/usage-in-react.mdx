...
Genqlx methods are just functions that return promises, to use genqlx in a react application you need a query manager like `swr` or `react-query`
...
```
npm i react-query @genqlx/runtime
```
