...
Genqlx queries are similar to graphql queries in structure, to select some fields you pass an object where the keys are the field names.
...
Genqlx let you query all scalar fields in a type by using the `everything` object:
...
Genqlx has an alternative way of executing queries called `chain` mode.
