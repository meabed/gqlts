...
export function runCommand({ cmd, cwd }) {
  return new Promise((res, rej) => {
    const ps = exec(cmd, { cwd, env: { ...process.env, HOME: os.tmpdir() } }, (err, stdout, stderr) => {
      if (err) {
        rej(err);
      }
      res(stdout);
...
export async function createPackage({
  ...
  try {
    ...
    await fs.writeFile(path.join(tmpPath, "package.json"), JSON.stringify(packageJson, null, 4));
    const readme = generateReadme({
      name: packageJson.name,
      schema: await fs.readFile(path.join(tmpPath, "schema.graphql"), "utf-8"),
    });
